...
void renderNode(SceneNode *mynode, bool material);
Matrix calTransform(SceneNode *mynode);
void flattenParseTree(SceneNode *mynode, Matrix& transform);
...
void callback_load(int id) {
	...
	if (success == false) {
		...
		sceneNodes.clear();
		Matrix tmp;
		flattenParseTree(parser->getRootNode(), tmp);
	}
}
...
void flattenParseTree(SceneNode *mynode, Matrix& transform) {
    ...
    for (int j = 0; j < size; j++ ) {
        flattenParseTree(mynode->children[j], tmpTrans);
    }
...
void myGlutDisplay(void)
{
	...
	if (wireframe) {
		...
		int nodeNum = sceneNodes.size();
		for (int j = 0; j < nodeNum; j++) {
		        glPushMatrix();   
                glMultMatrixd(sceneNodes[j].transform.unpack());
                renderShape(sceneNodes[j].obj);
                glPopMatrix();
