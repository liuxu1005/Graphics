	...
	glBegin(GL_POINTS);
	    while (cur != tail) {
	        ...
	        for (int i = 1; i <= resolution; i++) {
	            ...
	            float dt = float(i) * (cur->next->x - cur->x)/(resolution + 1);
	            
	             
	            computed_x_y_z[position * 3] =   cur->x + dt;
	            computed_x_y_z[position * 3 + 1] = calculate_Spline(dt, cur->y, cur->next->y, Vs, Vg);
	            computed_x_y_z[position * 3 + 2] = calculate_Spline(dt, cur->z, cur->next->z, Vs, Vg);
	       
	            
                glVertex3f(computed_x_y_z[position * 3], computed_x_y_z[position * 3 + 1], computed_x_y_z[position * 3 + 2]);
                position++;
